theme.formCompany_classEdit = function(params){
...
theme.formCompany_classGetRow = function(content){
            ...
            functionClick: function(event,me,index,parent,data,row){
                ...
                if (me === editIcon){
                   content.func.edit().then(function(value){
                       parent.updateRow(theme.formCompany_classGetRow(value), index);
                   });
                }
                if (me === deleteIcon){
...
theme.formCompany_classContentData = function(params){
    var data = [];
    for (var i = 0; i < params.data.length; i++){
        data.push(theme.formCompany_classGetRow(params.data[i]));
    }
...
theme.formCompany_classInit = function(params){
    var header = absol.buildDom({
        tag: 'mheaderbar',
        props: {
            actionIcon: DOMElement.i({
            ...
            title: LanguageModule.text("txt_company_class"),
            commands: [
                {
                    icon:  DOMElement.i({
                        attrs: {
